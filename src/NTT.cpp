...
// a is a std::vector of size 2^n
void fft(std::vector<unsigned> &a, int n, bool inverse=false) {
    int N = 1<<n;
    a.insert(a.end(), N-a.size(), 0);
    ...
    for(int i=0; i < N; i++) {
        int x=i,y=0;
        for(int j=0; j < n; j++)  y=(y<<1)+(x&1), x>>=1;
        if (i<y) std::swap(a[i],a[y]);
    }
    if (inverse) {
        unsigned inv = pw(N, P-2);
        for(int i=0; i < N; i++) a[i] = (ULL)a[i]*inv%P;
    }
}
...
std::vector<unsigned> conv(std::vector<unsigned> A, std::vector<unsigned> B) {
    ...
    fft(A, n);
    fft(B, n);
    for(int i=0; i < 1<<n; i++) A[i] = (ULL) A[i] * B[i] % P;
    fft(A, n, true);
    return A;
}
